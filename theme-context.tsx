...
interface ThemeContextType {
  ...
  replaceThemes: (configs: ThemeConfig[]) => void;
    ...
    try {
      ...
      const resolveAlias = (val: string) => {
        ...
        return val.replace(/\{([^{}]+)\}/g, (match, path) => {
          const cleanPath =
            path.startsWith("light.") || path.startsWith("dark.")
        ...
        Object.entries(obj).forEach(([key, value]) => {
          if (value && typeof value === "object" && "$value" in value) {
            scopeRef.current?.style.setProperty(
              `--${prefix}-${key}`,
              resolveAlias(String(value.$value)),
            );
    ...
    <div ref={scopeRef} className={className} style={{ display: "contents", ...style }}>
      {warnings.length > 0 && (
        ...
        >
          {warnings.map((w, i) => (
            <div key={i}>
              <strong>Accessibility Alert:</strong> {w}
            </div>
          ))}
        </div>
      )}
...
export const ThemeProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
        ...
        if (t.id === themeId) {
          return {
            ...
            colors: t.colors.map((c) =>
              c.id === colorId ? { ...c, name: safeName, seed: newSeed } : c,
            ),
          };
        }
        return t;
      ...
      prev.map((t) => {
        if (t.id === themeId) {
          ...
          return {
            ...
            colors: [
              ...t.colors,
              { id: newId, name: `color-${t.colors.length + 1}`, seed: "#000000" },
            ],
          };
        }
        return t;
  ...
  const updateThemeSemanticOverride = (
    themeId: string,
    path: string,
  ...
  const updateThemeGeometryValue = <K extends keyof ThemeGeometryConfig>(
    themeId: string,
    key: K,
    value: ThemeGeometryConfig[K],
  ) => {
  ...
  const replaceThemes = (configs: ThemeConfig[]) => {
    saveHistory();
    setThemes(configs);
  };
