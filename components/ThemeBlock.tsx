    ...
    <div style={{ marginBottom: "4rem" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "3rem" }}>
        {themeConfig.colors.map((color: any) => {
          ...
          const preferredStep = (
            isDarkMode ? 300 : gen.closestStep
          ) as import("../lib/palette-generator").ColorStep;
          ...
          if (contrast < 4.5) {
            return (
              <FeedbackCard key={`warning-${color.id}`} colorName="error" isDarkMode={isDarkMode}>
                <strong>Low Contrast ({color.name})!</strong> Primary button text ratio is only{" "}
                {contrast.toFixed(2)}:1. This fails WCAG AA standards.
              </FeedbackCard>
            );
          } else if (contrast < 7) {
            return (
              <FeedbackCard key={`warning-${color.id}`} colorName="neutral" isDarkMode={isDarkMode}>
                <strong>
                  AA Contrast ({color.name}) ({contrast.toFixed(2)}:1).
                </strong>{" "}
                Good, but consider adjusting for AAA (7:1).
              </FeedbackCard>
            );
          }
          return null;
