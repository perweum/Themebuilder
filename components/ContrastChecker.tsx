...
const ContrastRow: React.FC<{ label: string, fgVar: string, bgVar: string, isDarkMode: boolean, isLast?: boolean }> = ({ label, fgVar, bgVar, isDarkMode, isLast }) => {
    ...
    useEffect(() => {
        ...
        const timer = requestAnimationFrame(() => {
            ...
            if (bgColor && fgColor && bgColor !== 'rgba(0, 0, 0, 0)') { // wait for CSS to apply
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{
                    ...
                    <div>
                        ...
                        <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.875rem', color: 'var(--color-text-subtle)' }}>WCAG 2.1 pairings mapped to components</p>
                    </div>
                ...
                <div style={{ overflow: 'auto', maxHeight: '70vh', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '3rem' }}>

                    {/* Theme specific groups */}
                    {activeTheme?.colors.map(color => {
                        const cName = color.name.toLowerCase().replace(/\s+/g, '-');
                        const pairings = [
                            { label: 'Solid Button', bgVar: `--color-base-${cName}-default`, fgVar: `--color-text-${cName}-contrast` },
                            { label: 'Primary Text on Surface', bgVar: `--color-surface-${cName}-default`, fgVar: `--color-text-${cName}-default` },
                            { label: 'Primary Text on Subtle', bgVar: `--color-surface-${cName}-subtle`, fgVar: `--color-text-${cName}-default` },
                        ];

                        return (
                            <div key={color.id}>
                                <h3 style={{ fontSize: '1.125rem', fontWeight: 600, margin: '0 0 1rem 0', textTransform: 'capitalize' }}>{color.name}</h3>
                                <div style={{
                                    ...
                                    {pairings.map((p, i) => (
                                        <ContrastRow key={i} {...p} isDarkMode={isDarkMode} isLast={i === pairings.length - 1} />
                                    ))}
                                </div>
                            </div>
                        );
                    ...
                    <div>
                        ...
                        <div style={{
                            ...
                            <ContrastRow label="Canvas bg + Text default" bgVar="--color-background-default" fgVar="--color-text-default" isDarkMode={isDarkMode} />
                            <ContrastRow label="Canvas bg + Text subtle" bgVar="--color-background-default" fgVar="--color-text-subtle" isDarkMode={isDarkMode} />
                            <ContrastRow label="Surface bg + Text default" bgVar="--color-surface-default" fgVar="--color-text-default" isDarkMode={isDarkMode} />
                            <ContrastRow label="Surface bg + Text subtle" bgVar="--color-surface-default" fgVar="--color-text-subtle" isDarkMode={isDarkMode} isLast />
                        </div>
                    </div>
                </div>
            </div>
