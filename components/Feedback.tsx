...
export const FeedbackCard: React.FC<FeedbackCardProps> = ({ colorName, isDarkMode, children }) => {
    ...
    const style: React.CSSProperties = {
        background: isSemantic ? `var(--color-background-${safeName})` : `var(--${cssPrefix}-${bgStep})`,
        color: isSemantic ? `var(--color-text-${safeName}-default)` : `var(--${cssPrefix}-${textStep})`,
        border: isSemantic ? `var(--geometry-borderWidth-default, 1px) solid var(--color-border-${safeName}-default)` : `var(--geometry-borderWidth-default, 1px) solid var(--color-border-subtle)`,
