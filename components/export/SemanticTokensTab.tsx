...
export const SemanticTokensTab: React.FC<SemanticTokensTabProps> = ({
  ...
  setExcludedSemanticCategories,
  excludedSemanticTokens,
  setExcludedSemanticTokens,
}) => {
  const {
    themes,
    globalColors,
  ...
  const tokenCategories = useMemo(() => {
    ...
    const globalNames = new Set(
      globalColors.map((gc) => gc.name.toLowerCase().replace(/\s+/g, "-")),
    );
    ...
    <>
        ...
        return (
          <optgroup label={c.name} style={{ textTransform: "capitalize" }} key={c.id}>
            {COLOR_STEPS.map((step: any) => (
              <option key={`${cName}-${step}`} value={`{color.${cName}.${step}}`}>
                {cName}-{step}
              </option>
            ))}
          </optgroup>
      ...
      <optgroup label="White (Alpha)">
        {ALPHA_STEPS.map((step: any) => (
          <option key={`white-${step}`} value={`{color.white.${step}}`}>
            white-{step}
          </option>
        ))}
      </optgroup>
      <optgroup label="Black (Alpha)">
        {ALPHA_STEPS.map((step: any) => (
          <option key={`black-${step}`} value={`{color.black.${step}}`}>
            black-{step}
          </option>
        ))}
      </optgroup>
      {globalColors.map((gc) => {
        ...
        return (
          <optgroup label={gc.name} style={{ textTransform: "capitalize" }} key={gc.id}>
            {COLOR_STEPS.map((step: any) => (
              <option key={`${safeName}-${step}`} value={`{color.${safeName}.${step}}`}>
                {safeName}-{step}
              </option>
            ))}
          </optgroup>
        ...
        {["Token Name", "Light Mode", "Dark Mode", "Export"].map((h, i) => (
          <div
            ...
            style={{
              ...
              textTransform: "uppercase",
              letterSpacing: "0.05em",
              justifySelf: i === 3 ? "center" : undefined,
            }}
          >
            {h}
          </div>
            ...
            {isExpanded && (
              ...
              >
                {paths.map((path) => {
                  ...
                  const currentLightVal = getValueByPath(
                    activeThemePayloadWithOptions,
                    `theme.${path}`,
                  );
                  const currentDarkVal = getValueByPath(
                    activeThemePayloadWithOptions,
                    `darkTheme.${path}`,
                  );
                  ...
                  if (lightHex?.startsWith("{"))
                    lightHex =
                      getValueByPath(activeThemePayloadWithOptions, lightHex.slice(1, -1)) ||
                      lightHex;
                  let darkHex = currentDarkVal;
                  if (darkHex?.startsWith("{"))
                    darkHex =
                      getValueByPath(activeThemePayloadWithOptions, darkHex.slice(1, -1)) ||
                      darkHex;
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <div
                          style={{ fontSize: "0.75rem", color: isDarkMode ? "#94a3b8" : "#64748b" }}
                        >
                          {path}
                        </div>
                      </div>
                      ...
                      >
                        <div
                          style={{
                            ...
                            height: "24px",
                            borderRadius: "4px",
                            backgroundColor: lightHex || "transparent",
                        ...
                        <select
                          value={
                            isLightOverridden
                              ? activeTheme.semanticOverrides![lightOverridePath]
                              : currentLightVal || ""
                          }
                          onChange={(e) =>
                            updateThemeSemanticOverride(
                          ...
                          style={selectStyle(isLightOverridden)}
                        >
                          {!isLightOverridden && (
                      ...
                      >
                        <div
                          style={{
                            ...
                            height: "24px",
                            borderRadius: "4px",
                            backgroundColor: darkHex || "transparent",
                            border: "1px solid #ccc",
                            flexShrink: 0,
                          }}
                        />
                        <select
                          value={
                            isDarkOverridden
                              ? activeTheme.semanticOverrides![darkOverridePath]
                              : currentDarkVal || ""
                          }
                          onChange={(e) =>
                            updateThemeSemanticOverride(
                          ...
                          style={selectStyle(isDarkOverridden)}
                        >
                          {!isDarkOverridden && (
