...
export const PrimitiveTokensTab: React.FC<PrimitiveTokensTabProps> = ({
    isDarkMode, excludedPalettes, setExcludedPalettes
}) => {
    ...
    const themeNames = themes[0]?.colors.map((c: any) => c.name.toLowerCase().replace(/\s+/g, '-')) ?? [];
    ...
    if (colors.white && !seen.has('white')) orderedPalettes.push({ name: 'white', steps: colors.white, label: 'White (Alpha)' });
    if (colors.black && !seen.has('black')) orderedPalettes.push({ name: 'black', steps: colors.black, label: 'Black (Alpha)' });
            ...
            {orderedPalettes.map(({ name, steps, label }) => (
                <div key={name}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', borderBottom: `1px solid ${isDarkMode ? '#334155' : '#e2e8f0'}`, paddingBottom: '0.5rem', marginBottom: '0.75rem' }}>
                        ...
                        />
                        <h3 style={{ textTransform: 'capitalize', fontSize: '1rem', margin: 0, color: isDarkMode ? '#f8fafc' : '#0f172a', opacity: excludedPalettes.has(name) ? 0.5 : 1 }}>
                            {label}
                        </h3>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))', gap: '0.5rem', opacity: excludedPalettes.has(name) ? 0.3 : 1, pointerEvents: excludedPalettes.has(name) ? 'none' : 'auto' }}>
