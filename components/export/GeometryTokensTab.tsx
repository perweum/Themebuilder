...
export const GeometryTokensTab: React.FC<GeometryTokensTabProps> = ({
    isDarkMode, excludedGeometry, setExcludedGeometry
}) => {
    ...
    return (
        <>
            {geoCategories.map((category) => {
                ...
                return (
                    <div key={category}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', borderBottom: `1px solid ${isDarkMode ? '#334155' : '#e2e8f0'}`, paddingBottom: '0.5rem', marginBottom: '0.75rem' }}>
                            <input
                                type="checkbox"
                            ...
                            />
                            <h3 style={{ textTransform: 'capitalize', fontSize: '1rem', margin: 0, color: isDarkMode ? '#f8fafc' : '#0f172a', opacity: excludedGeometry.has(category) ? 0.5 : 1 }}>
                                {category === 'radius' ? 'Border Radius' : 'Border Width'}
                            </h3>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '0.5rem', opacity: excludedGeometry.has(category) ? 0.3 : 1, pointerEvents: excludedGeometry.has(category) ? 'none' : 'auto' }}>
                                ...
                                return (
                                    <div key={stepKey} style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', padding: '0.5rem', background: isDarkMode ? '#1e293b' : '#f8fafc', borderRadius: '4px' }}>
                                        ...
                                        {!isAlias && category === 'radius' && (
                                            <div style={{ height: '24px', backgroundColor: isDarkMode ? '#334155' : '#e2e8f0', borderRadius: valObj.$value, border: `1px solid ${isDarkMode ? '#475569' : '#cbd5e1'}` }} />
                                        )}
                                        {!isAlias && category === 'borderWidth' && (
                                            <div style={{ height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: isDarkMode ? '#0f172a' : '#fff' }}>
                                                <div style={{ width: '100%', borderTop: `${valObj.$value} solid ${isDarkMode ? '#94a3b8' : '#64748b'}` }} />
                                            </div>
                                        )}
                                    </div>
                                );
