...
import { TextField, Label, Input as AriaInput, Text, TextFieldProps, Switch, SwitchProps, Checkbox, CheckboxProps, RadioGroup, RadioGroupProps, Radio, RadioProps } from 'react-aria-components';
...
export const Input: React.FC<CustomInputProps> = ({ label, placeholder, error, errorMessage, colorName = 'neutral', ...props }) => {
    return (
        <TextField {...props} isInvalid={error} style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', width: '100%' }}>
            {label && (
                <Label style={{
                    fontSize: '0.875rem',
                    fontWeight: 600,
            ...
            <AriaInput
                ...
                style={({ isFocused, isFocusVisible, isInvalid, isHovered }) => {
                    let borderColor = colorName === 'neutral' ? 'var(--color-border-default)' : `var(--color-border-${colorName}-default)`;
                    ...
                    if (isInvalid) {
                        ...
                        borderColor = colorName === 'neutral' ? 'var(--color-border-focus)' : `var(--color-border-${colorName}-hover)`;
                        addInnerShadow = true;
                    }

                    let shadowItems = [];
                    if (addInnerShadow) shadowItems.push(`inset 0 0 0 1px ${borderColor}`);
                    if (isFocusVisible) shadowItems.push('0 0 0 2px var(--color-background-default), 0 0 0 4px var(--color-border-focus)');
            ...
            />
            {error && errorMessage && (
                <Text slot="errorMessage" style={{ fontSize: '0.75rem', color: 'var(--color-text-error)', marginTop: '0.25rem' }}>
                    {errorMessage}
                </Text>
            )}
        </TextField>
...
export const Toggle: React.FC<CustomToggleProps> = ({ checked: initialChecked = false, colorName = 'brand', ...props }) => {
    return (
        <Switch
                        ...
                        <div style={{
                            ...
                            {isSelected && (
                                <svg width="14" height="14" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M10 3L4.5 8.5L2 6" stroke={`var(--color-text-${colorName}-contrast)`} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                </svg>
                            )}
                        </div>
                        {label}
...
export const CustomRadioGroup: React.FC<RadioGroupProps & { label: string }> = ({ label, children, style, ...props }) => {
    return (
        <RadioGroup {...props} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', ...(typeof style === 'object' ? style : {}) }}>
            <Label style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--color-text-subtle)' }}>{label}</Label>
            {children as any}
        </RadioGroup>
    );
};
