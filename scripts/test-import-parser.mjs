...
function mineScales(obj, depth = 0) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (detectScale(value)) {
      ...
      for (const [k, s] of mineScales(value, depth + 1)) if (!result.has(k)) result.set(k, s);
    }
  }
  return result;
}
...
function mineIndividual(obj, depth = 0) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (hex) {
      if (!result.has(key)) result.set(key, { hex, step: "-", method: "individual" });
    } else if (typeof value === "object")
      for (const [k, s] of mineIndividual(value, depth + 1)) if (!result.has(k)) result.set(k, s);
  }
  return result;
}
...
function parseJson(json) {
  if (json["$themebuilder"]?.version === "1.0") {
    ...
    return {
      ...
      colors:
        t?.colors?.map((c) => ({ name: c.name, hex: c.seed, step: "seed", method: "metadata" })) ??
        [],
      globals: json["$themebuilder"].globalColors?.map((g) => g.name) ?? [],
      warnings: [],
    };
  }
  ...
  if (scales.size > 0) {
    ...
    const fmt = json["$themes"]
      ? "token-studio"
      : json.color && (json.theme || json.darkTheme)
        ? "themebuilder-raw"
        : "generic";
  ...
  if (individuals.size > 0) {
    ...
    return {
      ...
      colors,
      globals,
      warnings: ["no scales found — individual colors only"],
    };
  }
...
function test(label, json, expect) {
  ...
  const ok_seedApprox =
    !expect.seedApprox ||
    expect.seedApprox.every(({ name, contains }) => {
      const c =
        result.colors.find((c) => c.name === name) || result.globals.find((c) => c.name === name);
  ...
  console.log(
    `   colors  : ${result.colors.map((c) => `${c.name}(${c.hex} via ${c.method}@${c.step})`).join(", ") || "(none)"}`,
  );
  ...
  if (!ok) {
    ...
    if (!ok_colors)
      console.log(`   ⚠ missing colors: ${expect.colors?.filter((n) => !colorNames.includes(n))}`);
    if (!ok_globals)
      console.log(
        `   ⚠ missing globals: ${expect.globals?.filter((n) => !globalNames.includes(n))}`,
      );
    if (!ok_noExtra)
      console.log(
        `   ⚠ unexpected colors: ${colorNames.filter((n) => !expect.colors?.includes(n))}`,
      );
  }
}
...
test(
  "1. Themebuilder v1 — perfect round-trip",
  {
    color: {},
    theme: {},
    darkTheme: {},
    ...
    $themebuilder: {
      ...
      theme: {
        ...
        name: "My Brand",
        colors: [
          { id: "c1", name: "brand", seed: "#014592" },
          { id: "c2", name: "accent", seed: "#ffbad9" },
        ],
        geometry: {},
        fontFamily: "Inter",
      },
...
test(
  "2. Token Studio ($themes) — our export format",
  {
    $themes: [
      {
        id: "light",
    ...
    Light: {
      color: { background: { default: { $value: "{Primitives.color.brand.25}", $type: "color" } } },
    },
    Dark: {
      color: {
        background: { default: { $value: "{Primitives.color.brand.900}", $type: "color" } },
      },
    },
  },
...
test(
  "3. Tailwind palette — 50-900 steps, raw hex strings",
  {
    dominant: {
      50: "#eff6ff",
      100: "#dbeafe",
  ...
  },
  {
    ...
    globals: ["neutral"],
    seedApprox: [
      { name: "dominant", contains: "3b82f6" },
      { name: "secondary", contains: "22c55e" },
    ],
  },
);
...
test(
  "4. Material Design — 100-900 + A100/A200/A400/A700",
  {
    primary: {
      100: "#bbdefb",
      200: "#90caf9",
...
test(
  "5. Single seed colors — dominant/secondary flat hex",
  {
    dominant: "#4f46e5",
    secondary: "#10b981",
    neutral: "#64748b",
...
test(
  ...
  },
  {
    ...
    colors: ["brand", "accent"],
    seedApprox: [
      { name: "brand", contains: "4f46e5" },
      { name: "accent", contains: "10b981" },
    ],
  },
);
...
test(
  "7. Radix UI style — 1-10, light step-5 rejected",
  {
    violet: {
      1: "#fdfcfe",
      2: "#faf8ff",
  ...
  },
  {
    ...
    colors: ["violet", "green"],
    seedApprox: [
      { name: "violet", contains: "6e56cf" },
      { name: "green", contains: "3d9a50" },
    ],
  },
);
...
test(
  "8. Style Dictionary — nested color groups",
  {
    color: {
      brand: {
  ...
  },
  {
    ...
    globals: ["error", "success", "warning"],
  },
);

// 9. Deeply nested with noise (spacing, typography mixed in)
test(
  "9. Mixed tokens — colors buried among non-color tokens",
  {
    typography: {
      fontFamily: {
...
test(
  "10. Alias-heavy file — aliases skipped, real hex still found",
  {
    primitive: {
      coral: {
        300: { $value: "#fca5a5", $type: "color" },
        500: { $value: "#f87171", $type: "color" },
        700: { $value: "#dc2626", $type: "color" },
      },
    },
    semantic: {
...
test(
  "11. Old Figma Tokens format — value (non-$)",
  {
    global: {
      brand: {
        100: { value: "#dbeafe", type: "color" },
...
test(
  "12. Scale with aliased 500 — luminance fallback kicks in",
  {
    brand: {
      100: { $value: "#dbeafe", $type: "color" },
      300: { $value: "#93c5fd", $type: "color" },
...
test(
  "13. Unknown format — plain color map, no scale structure",
  {
...
test(
  "14. CSS variable names — dash-prefixed keys",
  {
    colors: {
      primary: "#4f46e5",
      accent: "#10b981",
...
test(
  "15. Multi-brand Token Studio — two brand color sets",
  {
    $themes: [
      {
        id: "brand-a-light",
        name: "Brand A Light",
        selectedTokenSets: { "brand-a": "source", base: "source", "light-semantic": "enabled" },
      },
      {
        id: "brand-b-light",
        name: "Brand B Light",
        selectedTokenSets: { "brand-b": "source", base: "source", "light-semantic": "enabled" },
      },
    ],
    $metadata: { tokenSetOrder: ["brand-a", "brand-b", "base", "light-semantic"] },
    "brand-a": {
      primary: { 100: "#fce7f3", 300: "#f9a8d4", 500: "#ec4899", 700: "#be185d", 900: "#831843" },
    },
    "brand-b": {
      primary: { 100: "#e0f2fe", 300: "#7dd3fc", 500: "#0ea5e9", 700: "#0369a1", 900: "#0c4a6e" },
    },
    base: {
      neutral: { 100: "#f5f5f5", 300: "#d4d4d4", 500: "#737373", 700: "#404040", 900: "#171717" },
    },
